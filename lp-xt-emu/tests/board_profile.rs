...
use lp_xt_emu::emu::{
    RunOutcome, CODE_DBUS_BASE, CODE_REGION_LEN, INITIAL_SP, STACK_DBUS_BASE, STACK_REGION_LEN,
};
use lp_xt_emu::memory::{AliasRule, Memory, EXC_INSTR_FETCH_ERROR};
...
fn classic_sram1_is_word_mirrored_not_linear() {
    ...
    assert_eq!(emu.mem.read_u32(p.alias.dbus_to_ibus(d0)).unwrap(), 0xC0DE_0000);
    assert_eq!(emu.mem.read_u32(p.alias.dbus_to_ibus(d1)).unwrap(), 0xC0DE_0100);
