...
use lp_xt_inst::{
    b4const_index, b4constu_index, decode, disasm::l32r_target, encode, AluRrr, BrRr, BrZ, CallOp,
    Inst, LoadOp, Reg, StoreOp, B4CONST, B4CONSTU,
};
use xt_mini_emit::imm::{extui_legal, fallback, is_legal, spec, Fallback, ImmOp, ImmRule, PcRel};
...
fn range_boundaries_accept_and_reject() {
    for &op in ImmOp::ALL {
        if let ImmRule::Range { min, max, step } = spec(op).rule {
            ...
            assert!(!is_legal(op, min - step), "{op:?}: below min must be illegal");
            assert!(!is_legal(op, max + step), "{op:?}: above max must be illegal");
            if step > 1 {
...
fn pinned_ranges_match_assembler_probes() {
    ...
    let cases: &[(ImmOp, &[i32], &[i32])] = &[
        ...
        (ImmOp::Addmi, &[-32768, -256, 0, 256, 32512], &[-33024, -255, 255, 32768]),
        ...
        (ImmOp::L32rDisp, &[-262144, -4], &[-262148, -262143, -3, 0, 4]),
        ...
        (ImmOp::CallDisp, &[-524288, 0, 4, 524284], &[-524292, -2, 2, 524288]),
    ];
