//! `led-lab-esp32s3` — the real-hardware backend for `lp-ws281x`.
//!
//! Drives **all four** ESP32-S3 RMT TX channels concurrently, each with its own
...
//!
//! ```text
//! E1: MEASURE rmt_base=0x60016000 rmt_ram=0x60016800 ram_offset=0x800 ...
...
//! | Signal | GPIO | Notes |
//! |--------|------|-------|
//! | Channel 0 data | 4 | RMT `CH0` via the GPIO matrix |
//! | Channel 1 data | 5 | RMT `CH1` |
//! | Channel 2 data | 6 | RMT `CH2` |
//! | Channel 3 data | 7 | RMT `CH3` |
//! | Debug | 15 | High for the duration of channel 0's frame; three fast pulses when a guard trips |
//!
//! # Start modes
//!
//! The demo alternates between the two ways four outputs get used, because they
//! stress different things:
//!
//! * **simultaneous** — all four frames start within a few register writes of
//!   each other, so every channel crosses its half boundaries in lockstep and
//!   the handler is entered with up to four coincident thresholds;
...
//! * [`s3_rmt::S3Rmt`] — the seven register operations, and the only place in
//!   this firmware that knows an ESP32-S3 address.
...
use s3_rmt::{S3Rmt, TX_BLOCKS, TX_CHANNELS};
#[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
use s3_rmt::{BLOCKS_PER_CHANNEL, CHANNEL_WORDS, RAM_BASE, RAM_OFFSET};
...
/// marker. The other three are its equals in every other respect. (The loopback
/// harness addresses all four by index and has no use for it.)
#[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
const CH: u8 = 0;

/// LEDs per channel — deliberately unequal, so the four frames end at four
/// different times and the handler never settles into a single rhythm.
#[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
...
/// the phase relationship between the four channels keeps drifting.
#[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
...
static DRIVER: Ws281xDriver<S3Rmt, TX_CHANNELS> =
...
///
/// One entry can service all four channels: with a block each they cross their
/// half boundaries within microseconds of one another, so coincident causes are
/// the rule rather than the exception. That single pass is
...
/// Per-channel wire configuration: four different strips on one peripheral.
///
/// Channel 0 keeps the plain WS2812/GRB setup the golden vectors were captured
/// with; the rest exist to prove the configuration is genuinely per channel and
/// not a global the handler happens to read.
#[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
...
enum Mode {
    /// All four channels started together, then waited for together.
    Simultaneous,
...
/// The demo: four independent chases on GPIO4–7 plus the `E1`/`E2` serial
/// protocol.
#[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
fn demo(peripherals: esp_hal::peripherals::Peripherals) -> ! {
    esp_println::println!(
        "led-lab-esp32s3: ws281x RMT driver, {} channels on GPIO4-7 ({} LEDs), debug on GPIO15",
        TX_CHANNELS,
        STRIP_LEDS[0],
    );
    ...
    let mut debug = Output::new(peripherals.GPIO15, Level::Low, OutputConfig::default());
    ...
    // release that channel's memory blocks and disconnect its pin. Owning them
    ...
    let probe = s3_rmt::probe_ram_address(&TX_BLOCKS, CH, DIRECT_SENTINEL, FIFO_SENTINEL);
    esp_println::println!(
    ...
    for (ch, timing) in timings.iter().enumerate() {
        if let Err(e) = DRIVER.configure_default_clock(ch as u8, timing) {
            esp_println::println!("E2: FAIL ws281x_s3_basic reason=configure_ch{ch}:{e:?}");
            halt();
        }
    }
    ...
    loop {
        match mode {
            Mode::Simultaneous => {
                ...
                let starts: [(u8, &[u8]); TX_CHANNELS] = [
                    ...
                    (2, &storage[2][..STRIP_LEDS[2] * 3]),
                    (3, &storage[3][..STRIP_LEDS[3] * 3]),
                ];
                ...
                if let Err(e) = send {
                    esp_println::println!("E2: FAIL ws281x_s3_basic reason=start:{e:?}");
                }
        ...
        if last_report.elapsed() >= REPORT_INTERVAL {
            ...
            if !advancing {
                esp_println::println!(
                    "E2: FAIL ws281x_s3_basic reason=frames_stalled mode={}",
                    mode.name()
                );
            } else if trips != 0 {
                ...
                esp_println::println!(
                    "E2: FAIL ws281x_s3_basic reason=idle_guard_trip guard_trips_delta={trips} \
                     mode={}",
                    mode.name()
                );
            } else if errs != 0 {
                esp_println::println!(
                    "E2: FAIL ws281x_s3_basic reason=tx_err errors_delta={errs} mode={}",
                    mode.name()
                );
            } else if hangs != 0 {
                esp_println::println!(
                    "E2: FAIL ws281x_s3_basic reason=frame_timeout timeouts_delta={hangs} mode={}",
                    mode.name()
                );
            } else {
                esp_println::println!(
                    "E2: PASS ws281x_s3_basic channels={} frames_advancing=1 mode={}",
                    TX_CHANNELS,
                    mode.name(),
                );
            }
...
/// so four strips side by side are told apart at a glance.
#[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
...
fn panic(info: &core::panic::PanicInfo) -> ! {
    #[cfg(feature = "test_loopback")]
    esp_println::println!("E4: FAIL loopback_s3_x4 reason=panic info={info}");
    #[cfg(feature = "test_stress")]
    esp_println::println!("E6: FAIL stress_s3 reason=panic info={info}");
    #[cfg(not(any(feature = "test_loopback", feature = "test_stress")))]
    esp_println::println!("E2: FAIL ws281x_s3_basic reason=panic info={info}");
