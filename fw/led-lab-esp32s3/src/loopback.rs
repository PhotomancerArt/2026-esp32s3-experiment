//! P4 — four-channel RMT loopback self-test: the no-oscilloscope timing oracle,
//! now with every TX channel running at once.
//!
//! Routes each of the four TX waveforms (GPIO4/5/6/7) into its own RMT **RX
//! channel** (4/5/6/7) through the GPIO matrix — no wires, no strips required —
//! captures every (level, duration) pair at 12.5 ns resolution, and asserts the
//! WS281x wire protocol numerically while all four channels transmit
//! *simultaneously*:
//!
...
//!
//! This supersedes P3's single-channel harness: every assertion it made about
//! channel 0 is still made here, on channel 0, plus the concurrency.
//!
...
//! pins still sees the frames; nothing needs to be attached.
//!
//! # RX capacity — the P4 constraint
//!
//! With four TX channels taking one memory block each, the four RX channels get
//! **one block each too**: 48 capture items, against P3's luxury of all four
//! blocks (192) for a single receiver. 48 items is exactly 48 bits — two LEDs —
//! because the ESP32-S3 records the over-idle-threshold trailing low as a
...
use crate::s3_rmt::{self, BLOCKS_PER_CHANNEL, TX_BLOCKS};
...
///
/// P3 measured every captured bit exactly on its nominal value at 12.5 ns
/// resolution, so the tight bound costs nothing and catches more. P4 keeps it
/// with four channels running — if concurrency perturbed the waveform at all,
...
/// the golden vectors in `lp-ws281x/tests/golden/` were captured with.
...
pub fn run(peripherals: Peripherals) -> ! {
    esp_println::println!(
        "led-lab-esp32s3: P4 loopback self-test, GPIO4-7 TX ch0-3 -> RX ch4-7, no wires"
    );
    ...
    for ch in 0..CHANNELS {
        ...
        // WS2812/GRB frame P3 recorded, now taken with three other channels
        // transmitting alongside it.
        if ch == 0 {
            esp_println::println!(
