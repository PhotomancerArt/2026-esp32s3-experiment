//! P5b — two-channel RMT loopback self-test on the ESP32-C6: the
//! no-oscilloscope timing oracle, with both TX channels running at once.
//!
//! Routes each TX waveform (GPIO18/GPIO20) into its own RMT **RX channel**
//! (2/3) through the GPIO matrix — no wires, no strips required — captures every
//! (level, duration) pair at 12.5 ns resolution, and asserts the WS281x wire
//! protocol numerically while both channels transmit *simultaneously*:
//!
//! * every bit classifies cleanly as 0/1 and the decoded bytes equal the sent
//!   bytes, per channel, under two different configurations (WS2812/GRB and
//!   WS2812/RGB — same wire timing, different byte order, so a channel reading
//!   its neighbour's colour-order config is caught);
//! * per-bit high time and period sit within ±25 ns of *that channel's*
//!   configuration;
//! * neither channel ever decodes to the other's pattern (cross-talk);
//! * the trailing low is at least the configured 300 µs latch (as far as the
//!   idle-threshold capture can bound it);
//! * a 100-frame concurrent soak decodes clean on both channels with zero guard
//!   trips;
//! * a threshold interrupt suppressed on **one** channel truncates that channel
//!   at exactly its guard word while the other, sharing the same handler
//!   entries, finishes its frame intact.
//!
//! This is the S3's P4 harness scaled to the C6's two-TX/two-RX split; every
//! assertion it makes about channel 0 is still made here, on channel 0.
//!
//! # The C6's channel split
//!
//! The S3 pairs TX `n` with RX `n + 4`. The C6 has only four channels total —
//! `CH0`/`CH1` transmit, `CH2`/`CH3` receive, fixed in hardware — so TX `n`
//! pairs with RX `n + 2`, and the RX side has exactly as much RAM as the TX
//! side. There is no configuration in which a C6 receives on more than two
//! channels, so the 4×4 shape simply does not exist here.
//!
...
//! same pad. The output half drives the RMT TX channel exactly as the demo
//! does; the input half feeds the paired RX channel's input through the GPIO
//! matrix with the pad's input buffer enabled. Anything physically wired to the
//! pins still sees the frames; nothing needs to be attached. The C6's GPIO
//! matrix is the same chip-generic esp-hal API as the S3's — nothing about this
//! routing is Xtensa-specific.
//!
//! # RX capacity
//!
//! With both TX channels taking one memory block each, the two RX channels get
//! one block each too: 48 capture items, which is exactly 48 bits — two LEDs —
//! because the hardware records the over-idle-threshold trailing low as a
//! zero-duration marker inside the final bit's own item rather than as an extra
//! one. The test frames are sized accordingly (1–2 LEDs).
//!
//! Longer captures are still possible and the truncation test uses one: the C6
//! RMT has RX wrap (`rmt.has_rx_wrap` is true for `esp32c6` in
//! `esp-metadata-generated`), so esp-hal copies out half a window at a time on
//! `RMT_CH_RX_THR_EVENT` as long as the transaction is polled inside the
//! 24-item (30 µs) window. The poll loop below does nothing else, but that
...
//! (375 µs) — above the 300 µs latch and inside `CH_RX_CONF0.idle_thres`'s
//! 15-bit field, so the capture is only terminated by genuine end-of-frame idle
//! and the recorded trailing low bounds the latch from below. The esp-hal
//! blocking RX transaction polls `INT_RAW` directly, which is precisely why
//! [`crate::c6_rmt`] masks its interrupt clears down to the two TX bits per
//! field: on this chip the RX causes are interleaved *between* them.
...
use crate::c6_rmt::{self, BLOCKS_PER_CHANNEL, TX_BLOCKS};
use crate::{install_isr, DRIVER, FRAME_TIMEOUT, RMT_CLOCK};

/// TX channels under test, each paired with RX channel `ch + 2`.
const CHANNELS: usize = 2;

/// The RX channel driven by TX channel `ch`.
const fn rx_channel(ch: usize) -> usize {
    ch + CHANNELS
}

/// RX memory blocks per receive channel. Two receivers, two RX blocks: one
...
///
/// The S3 measured every captured bit exactly on its nominal value at 12.5 ns
/// resolution; the same bound is kept here rather than widened, so a C6 whose
/// RMT divider or clock source resolved differently fails loudly.
...
///
/// Channel 1, so channel 0 — the golden vector's channel — is the bystander.
const VICTIM: u8 = 1;
...
/// by that refill. Identical to the S3's, because the block size is identical
/// (48 words). Mirrored by `lp-ws281x/tests/hooks.rs` on the host.
...
/// LEDs per channel in the routine captures. Two lengths, so the channels stop
/// at different moments and a `tx_end` for one lands in the same interrupt
/// snapshot as a refill for the other.
const TEST_LEDS: [usize; CHANNELS] = [2, 1];
...
/// the golden vector in `lp-ws281x/tests/golden/` is captured with.
...
/// Transmit on every channel at once while capturing both; hand back the
...
fn capture_all<'ch>(
    ...
    let [t0, t1] = txns;
    let (Ok((n0, c0)), Ok((n1, c1))) = (t0.wait(), t1.wait()) else {
        return Err("rx_error");
    };
    Ok(([c0, c1], [n0, n1]))
}

/// Print `E5: FAIL` with `reason` forever — for failures that break the harness
/// itself rather than one assertion.
fn fatal(reason: &'static str) -> ! {
    loop {
        esp_println::println!("E5: FAIL loopback_esp32c6 reason={reason}");
...
fn starts<'a>(
    ...
    [(0, &frames[0][..lens[0]]), (1, &frames[1][..lens[1]])]
}

pub fn run(peripherals: Peripherals) -> ! {
    esp_println::println!(
        "led-lab-esp32c6: P5 loopback self-test, GPIO18/GPIO20 TX ch0-1 -> RX ch2-3, no wires"
    );
    ...
    let (rx_sig0, tx_sig0) = Flex::new(peripherals.GPIO18).split();
    let (rx_sig1, tx_sig1) = Flex::new(peripherals.GPIO20).split();
    ...
    let _tx_channels = match (
        ...
        (Ok(c0), Ok(c1)) => [c0.with_pin(tx_sig0), c1.with_pin(tx_sig1)],
    ...
    // `channel2`/`channel3` are the C6's only receivers — there is no chip
    // configuration in which `channel0`/`channel1` can be configured for RX.
    let rx = match (
        rmt.channel2.configure_rx(&rx_config),
        rmt.channel3.configure_rx(&rx_config),
    ) {
        (Ok(c2), Ok(c3)) => [c2.with_pin(rx_sig0), c3.with_pin(rx_sig1)],
    ...
    esp_println::println!(
        "E5: MEASURE routing option=1_flex_split gpios=18,20 tx_ch=0-1 rx_ch=2-3 \
    ...
    // --- Known-answer decode, per-bit timing, latch — both at once -----------
    for ch in 0..CHANNELS {
    ...
    for ch in 0..CHANNELS {
        ...
        // WS2812/GRB frame the S3 harness records, now measured by C6 silicon.
        if ch == 0 {
            esp_println::println!(
                "E5: MEASURE golden_begin chip=esp32c6 config=ws2812_grb clock_hz=80000000 \
    ...
    // --- Cross-talk: neither channel decoded to the other's pattern ----------
    // The two expectations must be distinct first, or the check below would be
    // vacuous.
    ...
    let soak_before: [_; CHANNELS] = [DRIVER.stats(0), DRIVER.stats(1)];
    let mut mismatches = [0usize; CHANNELS];
    let mut rx = rx;
    for f in 0..SOAK_FRAMES {
        for ch in 0..CHANNELS {
            // A different sequence per channel and per frame, so a stale half
            // or a swapped channel cannot alias into a plausible stream.
            for (j, byte) in frames[ch][..lens[ch]].iter_mut().enumerate() {
                *byte = ((f * 31 + j * 7 + ch * 97 + 3) % 251) as u8;
            }
            expected_len[ch] = wire_bytes(
                &frames[ch][..lens[ch]],
                timings[ch].color_order,
    ...
    for ch in 0..CHANNELS {
        ...
        esp_println::println!(
            "E5: MEASURE soak ch={} frames={} mismatches={} guard_trips={} guard_skips={} \
    ...
    if soak_ok {
        esp_println::println!(
            "E5: PASS loopback_soak frames={SOAK_FRAMES} channels={CHANNELS} concurrent=1"
        );
    } else {
        verdict.fail("soak");
        esp_println::println!("E5: FAIL loopback_soak frames={SOAK_FRAMES}");
    }

    // --- Truncation on one channel while the other runs ----------------------
    ...
    // reporting complete; the bystander shares every interrupt entry with it and
    // must finish its frame untouched.
    for ch in 0..CHANNELS {
        if ch as u8 == VICTIM {
            lens[ch] = TRUNC_LEDS * 3;
            for (j, byte) in frames[ch][..lens[ch]].iter_mut().enumerate() {
                *byte = ((j * 37 + 11) % 251) as u8;
            }
        } else {
            lens[ch] = TEST_LEDS[ch] * 3;
            frames[ch][..6].copy_from_slice(&TEST_FRAMES[ch]);
    ...
    let trunc_before: [_; CHANNELS] = [DRIVER.stats(0), DRIVER.stats(1)];
    ...
    for ch in 0..CHANNELS {
        ...
        if ch as u8 == VICTIM {
            ...
            //
            // `bits_written` is deliberately *not* asserted; it is reported
            // instead by the `thr_reraise` line below, which is the C6's answer
            // to the S3 finding that the hardware re-raises an unacknowledged
            // `tx_thr_event`.
    ...
    // --- Does the C6 re-raise an unacknowledged `tx_thr_event`? --------------
    // The S3 does: after the hook swallows one threshold, the peripheral raises
    // it again and the driver services that refill in the window between the
    // transmitter latching the guard word and the `tx_end` that reports it. The
    // refill writes into RAM the transmitter has already stopped reading, so it
    // is invisible on the wire, but the driver's bit cursor over-counts by one
    // half (24 bits) as a result.
    //
    // So: cursor == wire bits means no re-raise; cursor one half ahead means the
    // C6 behaves like the S3. Observed, never asserted — this is a fact about
    // the silicon, and pinning it would turn a future chip revision into a test
    // failure rather than a report. The driver is correct either way: the guard
    // word, not the cursor, is what stops the wire.
    let reraised = victim_bits_written > TRUNC_EXPECT_BITS;
    esp_println::println!(
        "E5: MEASURE thr_reraise chip=esp32c6 wire_bits={TRUNC_EXPECT_BITS} \
         cursor_bits={victim_bits_written} half_bits=24 reraised={} matches_esp32s3={}",
        reraised as u8,
        reraised as u8,
    );
    ...
    loop {
        if verdict.ok {
            esp_println::println!(
                "E5: PASS loopback_esp32c6 channels={CHANNELS} frames={frames_done}"
            );
        } else {
            esp_println::println!(
                "E5: FAIL loopback_esp32c6 first_fail={} frames={frames_done}",
                verdict.first_fail
            );
        }
        let park = Instant::now();
