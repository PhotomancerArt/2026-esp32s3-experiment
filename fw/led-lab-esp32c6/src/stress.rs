//! P6 — the stress harness on the ESP32-C6: does the refill interrupt survive
//! radio load?
//!
//! Both TX channels of long strips transmit back to back at the maximum rate the
//! wire allows while escalating load runs beside them. The question is whether a
//! refill interrupt at esp-hal's highest dispatchable priority ever arrives late
//! enough for the transmitter to reach its guard word — because if it does, the
//! fix is a higher-priority assembly shim, which is a whole follow-up plan.
//!
//! # C6 vs S3
//!
//! This is a port of `fw/led-lab-esp32s3/src/stress.rs`. What is genuinely
//! different, and why:
//!
//! * **Two channels, not four.** The C6's RMT has `CH0`/`CH1` for transmit and
//!   `CH2`/`CH3` for receive (see [`crate::c6_rmt`]), so half the S3's load can
//!   even be asked for. To keep the refill rate comparable, both strips are
//!   long — 300 and 256 LEDs — where the S3 tapered down to 150.
//! * **RISC-V, plain nightly.** `rust-toolchain.toml` here is upstream
//!   `nightly-2026-04-27`, not the Espressif `esp` channel, so none of the
//!   Xtensa-specific notes in the S3 module (level-4/5 shims, `-Zbuild-std`)
//!   transfer. The C6's interrupt priorities are 1..=15 and `Priority::max()`
//!   is dispatchable from Rust, so there is no priority ceiling to work around.
//! * **Pins GPIO18 (XIAO `D10`) and GPIO20 (`D9`)**, the same two the demo and
//!   loopback builds use.
//! * **Single core.** The C6 has one HP core, so the radio's tasks and this
//!   thread genuinely contend for the same CPU — which is the harsher test, and
//!   the reason this port is worth running rather than assuming the S3 result.
//!
...
//! | id | load |
//! |----|------|
//! | S0 | idle — no radio, no logging beyond the periodic report |
...
use crate::c6_rmt::{self, BLOCKS_PER_CHANNEL, TX_BLOCKS, TX_CHANNELS};
use crate::{install_isr, DRIVER, RMT_CLOCK};

/// LEDs per channel. Long strips on both channels: a 300-LED frame is 7200
/// bits, i.e. 300 refills at one LED per refill (the C6 gives each channel a
/// single 48-word block, so a half is 24 words = exactly one LED). The lengths
/// are unequal so the two channels' half-boundary crossings drift through every
/// phase relation instead of locking into one.
pub const STRIP_LEDS: [usize; TX_CHANNELS] = [300, 256];
...
/// `STRESS_SECONDS=25 cargo build …` shortens it for a smoke run.
...
/// Heap for the radio stack. Declared before S0 so that the quiet baseline runs
/// in exactly the memory layout the loaded scenarios will.
...
const fn build_frames() -> [[u8; MAX_FRAME_BYTES]; TX_CHANNELS] {
    ...
    while ch < TX_CHANNELS {
        ...
        while i < MAX_FRAME_BYTES {
            // A per-channel phase shift keeps the two wires visibly distinct.
...
pub fn run(peripherals: Peripherals) -> ! {
    esp_println::println!(
        "led-lab-esp32c6: P6 stress harness, {} channels on GPIO18/GPIO20, strips {:?}",
        TX_CHANNELS,
        STRIP_LEDS,
    );
    ...
    let mut rmt = match Rmt::new(peripherals.RMT, RMT_CLOCK) {
        Ok(rmt) => rmt,
        Err(e) => {
            esp_println::println!("E6: FAIL stress_c6 reason=rmt_init:{e:?}");
            crate::halt();
        }
    };
    install_isr(&mut rmt);
    ...
    let _channels = match (
        ...
        _ => {
            esp_println::println!("E6: FAIL stress_c6 reason=configure_tx");
            crate::halt();
        }
    };
    ...
    // Two wire configurations rather than the S3's four, for the same reason
    // the demo uses two: to prove the timing is genuinely per channel.
    let timings = [
    ...
    for (ch, timing) in timings.iter().enumerate() {
        if let Err(e) = DRIVER.configure_default_clock(ch as u8, timing) {
            esp_println::println!("E6: FAIL stress_c6 reason=configure_ch{ch}:{e:?}");
            crate::halt();
        }
    }
    ...
    // --- Bring the radio up. ---
    //
    // `wifi::new` already applies `ControllerConfig::initial_config`, which
    // defaults to `Config::Station(StationConfig::default())` *and* calls
    // `esp_wifi_start()` on the mode change — so the station is running by the
    // time this returns. Re-applying the identical config here would be a
    // no-op that skips the start (esp-radio only starts on a mode change), so
    // it is deliberately not done: `new` is the whole of station bring-up.
    let (mut controller, interfaces) =
        match esp_radio::wifi::new(peripherals.WIFI, Default::default()) {
            Ok(pair) => pair,
            Err(e) => {
                esp_println::println!("E6: FAIL stress_c6 reason=wifi_new:{e:?}");
                crate::halt();
            }
        };
    let mac = interfaces.station.mac_address();
    esp_println::println!(
        "E6: MEASURE radio=up mac={:02x}{:02x}{:02x}{:02x}{:02x}{:02x}",
        mac[0],
        mac[1],
        mac[2],
        mac[3],
    ...
    //
    // The future is dropped at the end of every iteration, which releases its
    // `EVENT_CHANNEL` subscriber; esp-radio's channel allows only two
    // subscribers (`ESP_RADIO_CONFIG_EVENT_CHANNEL_SUBSCRIBERS` defaults to 2)
    // and `scan_async` *panics* rather than erroring when it cannot get one, so
    // holding one across iterations would end the run.
    {
        ...
        while !soak.expired() {
            ...
            loop {
                ...
                match poll_once(scan.as_mut()) {
                    Some(Ok(aps)) => {
                        scans += 1;
                        esp_println::println!("L2: scan n={scans} aps={}", aps.len());
                        break;
                    }
                    Some(Err(e)) => {
                        esp_println::println!("L2: scan_err {e:?}");
                        break;
                    }
                    None => {}
                }
    ...
    esp_println::println!("E6: DONE stress_c6 scenarios=S0,S1,S2,S3");
    crate::halt()
}
...
impl Soak {
    ...
    /// Transmit one frame on both channels, simultaneously, and wait for the
    /// set to finish — ~9.4 ms for the 300-LED strip, ~560 refill interrupts.
    fn frame_batch(&mut self) {
        ...
        if let Err(e) = send {
            esp_println::println!("E6: FAIL stress_c6 reason=start:{e:?}");
        }
...
fn report(scenario: Scenario, secs: u64, timeouts: usize) {
    for ch in 0..TX_CHANNELS {
        let stats = DRIVER.stats(ch as u8);
        let half = DRIVER.channel(ch as u8).map(|c| c.half_words()).unwrap_or(0);
...
fn log_burst() {
    ...
    // SAFETY: single-threaded thread context on the C6's one HP core; the
    // counter is never touched by an interrupt handler, and the only other
    // scheduler task (the radio's) does not exist yet in S1.
    let seq = unsafe {
