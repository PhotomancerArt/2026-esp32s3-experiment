...
tests/
  ...
  hardware_golden.rs a frame as an ESP32-S3 actually transmitted it, decoded
                     ...
                     vectors; see fw/led-lab-esp32s3's README to re-derive)
```
...
(8/16/100/256 LEDs, `fw/led-lab-esp32s3`): mean read-pointer advance across a
...
the GPIO matrix at 12.5 ns resolution, `fw/led-lab-esp32s3` feature
`test_loopback`), and `tests/hardware_golden.rs` re-decodes them on the host.
