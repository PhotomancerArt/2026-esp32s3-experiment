...
pub fn encode(inst: &Inst) -> Vec<u8> {
    ...
    match *inst {
        Inst::Rrr(op, rd, rs, rt) => {
            ...
            emit(
                &mut out,
                pack(0, rt.num() as u32, rs.num() as u32, rd.num() as u32, op1, op2),
                3,
            );
        }
        Inst::Rt(op, rd, rt) => {
        ...
        Inst::Slli(rd, rs, shift) => {
            ...
            let t = field & 0xf;
            emit(&mut out, pack(0, t, rs.num() as u32, rd.num() as u32, 1, op2), 3);
        }
        Inst::Srli(rd, rt, sa) => {
            emit(
                &mut out,
        ...
        Inst::Srai(rd, rt, sa) => {
            ...
            let s = (sa & 0xf) as u32;
            emit(&mut out, pack(0, rt.num() as u32, s, rd.num() as u32, 1, op2), 3);
        }
        Inst::Extui(rd, rt, shiftimm, maskimm) => {
            ...
            emit(&mut out, pack(0, rt.num() as u32, s, rd.num() as u32, op1, op2), 3);
        }
        Inst::Sext(rd, rs, imm) => {
            let t = (imm - 7) as u32;
            emit(&mut out, pack(0, t, rs.num() as u32, rd.num() as u32, 3, 2), 3);
        }
        Inst::MovN(rt, rs) => {
            // RRRN op0=0xD, r=0, t=$t, s=$s
            emit(&mut out, narrow(0xd, rt.num() as u32, rs.num() as u32, 0), 2);
        }
        Inst::AddN(rd, rs, rt) => {
            emit(&mut out, narrow(0xa, rt.num() as u32, rs.num() as u32, rd.num() as u32), 2);
        }
        Inst::AddiN(rd, rs, imm) => {
            let t = if imm == -1 { 0 } else { (imm & 0xf) as u32 };
            emit(&mut out, narrow(0xb, t, rs.num() as u32, rd.num() as u32), 2);
        }
        Inst::Addi(rt, rs, imm) => {
            let imm8 = (imm as u32) & 0xff;
            emit(&mut out, rri8(2, rt.num() as u32, rs.num() as u32, 0xc, imm8), 3);
        }
        Inst::Addmi(rt, rs, imm) => {
            let imm8 = ((imm >> 8) as u32) & 0xff;
            emit(&mut out, rri8(2, rt.num() as u32, rs.num() as u32, 0xd, imm8), 3);
        }
        Inst::Movi(rt, imm) => {
        ...
        Inst::MoviN(rt, imm) => {
            let field = if imm < 0 { (imm + 128) as u32 } else { imm as u32 } & 0x7f;
        ...
        Inst::Load(op, rt, rs, offset) => {
            ...
            emit(&mut out, rri8(2, rt.num() as u32, rs.num() as u32, r, off_field & 0xff), 3);
        }
        ...
        Inst::L32iN(rt, rs, offset) => {
            ...
            emit(&mut out, narrow(0x8, rt.num() as u32, rs.num() as u32, off), 2);
        }
        Inst::S32iN(rt, rs, offset) => {
            let off = (offset / 4) & 0xf;
            emit(&mut out, narrow(0x9, rt.num() as u32, rs.num() as u32, off), 2);
        }
        Inst::L32r(rt, imm16) => {
        ...
        Inst::BranchRr(op, rs, rt, off) => {
            ...
            emit(&mut out, rri8(7, rt.num() as u32, rs.num() as u32, r, (off as u32) & 0xff), 3);
        }
        Inst::BranchRi(op, rs, val, off) => {
            ...
            let tnib = (m << 2) | 0x2;
            emit(&mut out, rri8(6, tnib, rs.num() as u32, idx, (off as u32) & 0xff), 3);
        }
        ...
        Inst::BranchZ(op, rs, off) => {
            ...
            let w = 6 | (1 << 4) | (m << 6) | ((rs.num() as u32) << 8) | (((off as u32) & 0xfff) << 12);
            emit(&mut out, w, 3);
        }
        Inst::BranchBiI(set, rs, imm, off) => {
            ...
            emit(&mut out, rri8(7, t, rs.num() as u32, r, (off as u32) & 0xff), 3);
        }
