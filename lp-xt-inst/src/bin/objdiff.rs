...
use lp_xt_inst::{decode, DecodeError, Inst};
use object::{Object, ObjectSection};
use std::collections::BTreeMap;
use std::process::Command;

const DEFAULT_OBJDUMP: &str =
    "/Users/yona/.rustup/toolchains/esp/xtensa-esp-elf/esp-14.2.0_20240906/xtensa-esp-elf/bin/xtensa-esp32s3-elf-objdump";
...
fn main() {
    ...
    for line in listing.lines() {
        ...
        match decode(bytes) {
            ...
            Err(DecodeError::Truncated { .. }) => {
                *unsupported.entry(format!("{od_mnem} (truncated)")).or_default() += 1;
            }
        }
    }
    ...
    println!("--- UNSUPPORTED allowlist ({} kinds) ---", unsupported.len());
...
fn typed_operands(inst: &Inst, pc: u32) -> Vec<Operand> {
    ...
    match *inst {
        ...
        Extui(a, b, sh, mk) => vec![reg(a), reg(b), Operand::Imm(sh as i64), Operand::Imm(mk as i64)],
...
fn parse_operands(text: &str, hints: &[Operand]) -> Option<Vec<Operand>> {
    let rest = text.split_once(char::is_whitespace).map(|(_, r)| r).unwrap_or("");
    let rest = rest.trim();
    if rest.is_empty() {
    ...
    for (tok, hint) in toks.iter().zip(hints) {
        ...
        let val = match hint {
            ...
            Operand::Addr(_) => Operand::Addr(u32::from_str_radix(tok.trim_start_matches("0x"), 16).ok()?),
        };
        out.push(val);
    }
    Some(out)
