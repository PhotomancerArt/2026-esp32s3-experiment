...
use lp_xt_inst::Inst;
use object::elf::{
    R_XTENSA_32, R_XTENSA_ASM_EXPAND, R_XTENSA_NONE, R_XTENSA_SLOT0_OP, SHF_ALLOC,
    SHF_EXECINSTR,
};
use object::{
...
impl Region {
    ...
    fn place(&mut self, align: u32, data: &[u8], size: u32) -> Result<u32, LinkError> {
        ...
        self.bytes.resize((addr - self.base) as usize + size as usize, 0);
        Ok(addr)
    }
    ...
    for (oi, file) in files.iter().enumerate() {
        for sec in file.sections() {
            ...
            for (off, reloc) in sec.relocations() {
                ...
                let s = match reloc.target() {
                    RelocationTarget::Symbol(sym_idx) => {
                        ...
                        if let Some(si) = sym.section_index() {
                            ...
                            let name = sym.name().map_err(|e| LinkError::Parse(e.to_string()))?;
                            *globals.get(name).ok_or_else(|| LinkError::UndefinedSymbol {
                                name: name.to_string(),
                            })?
                        }
                    }
                ...
                let region = if loc >= TEXT_BASE { &mut text } else { &mut data };
                match r_type {
                    R_XTENSA_32 => {
                        let slot = region
                            .slice_mut(loc, 4)
...
mod tests {
    ...
    fn retarget_call() {
        for (pc, target) in [
            ...
            assert_eq!((pc & !3).wrapping_add(4).wrapping_add((field as u32) << 2), target);
        }
    }
    ...
    fn retarget_l32r() {
        ...
        let Inst::L32r(_, field) = patched else { panic!("wrong variant") };
    ...
    fn retarget_branch_range() {
        ...
        let ok = retarget_slot0(&Inst::BranchZ(BrZ::Beqz, Reg::new(2), 0), pc, pc + 4 + 100)
            .unwrap();
        assert!(matches!(ok, Inst::BranchZ(BrZ::Beqz, _, 100)));
        let err = retarget_slot0(&Inst::BranchZ(BrZ::Beqz, Reg::new(2), 0), pc, pc + 5000)
            .unwrap_err();
        assert!(matches!(err, LinkError::OperandRange { .. }));
    }
