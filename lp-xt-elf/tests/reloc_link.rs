...
use lp_xt_elf::reloc::{link_objects, run_linked, LinkError};
...
fn layout_regions() {
    ...
    assert!((TEXT_BASE..TEXT_BASE + 0x1_0000).contains(&lp_main), "{lp_main:#x}");
    assert_eq!(lp_main % 4, 0, "call8 entry must be 4-aligned");
    assert!((DATA_BASE..DATA_BASE + 0x1_0000).contains(&table), "{table:#x}");
...
fn rejects_executable_input() {
    ...
    assert!(
        matches!(err, LinkError::NotXtensaRelocatable { object_index: 0, .. }),
        "got {err:?}"
    );
}
