...
#[test]
fn call_conv() {
    #[allow(clippy::too_many_arguments)] // mirrors the guest fixture exactly
    ...
    let bsum = (0..8u32).fold(0u32, |acc, i| {
        acc.wrapping_add(y.wrapping_mul(i + 1) ^ i)
    });
...
fn jump_table() {
    ...
    assert_fixture("jump_table", &format!("x={}\n", x));
}
