...
use lp_xt_elf::{abi, run_elf, ElfError, XtensaElf};
use lp_xt_emu::emu::CODE_DBUS_BASE;
use lp_xt_emu::memory::Memory;
use lp_xt_emu::{Emulator, RunOutcome};
use lp_xt_inst::{encode, CallOp, Inst, NullaryOp, Reg, StoreOp};
...
impl SynthElf {
    ...
    fn build(&self) -> Vec<u8> {
        ...
        if self.with_rela {
            ...
            // Section headers.
            let sh = |name: u32, ty: u32, addr: u32, off: u32, size: u32, link: u32,
                          info: u32, entsize: u32, out: &mut Vec<u8>| {
                le32(out, name);
                le32(out, ty);
                le32(out, 0); // sh_flags
                le32(out, addr);
            ...
            // .shstrtab (SHT_STRTAB)
            sh(18, 3, 0, shstr_off, shstrtab.len() as u32, 0, 0, 0, &mut out);
        }
...
fn rejects_unmapped_segment() {
    ...
    assert!(
        matches!(err, ElfError::Unmapped { vaddr: 0x6000_0000, .. }),
        "got {err:?}"
    );
}
